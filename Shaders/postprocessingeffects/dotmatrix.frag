...
vec2 pixelate_uv(const in vec2 screenUV, const in float pixel_size, const in vec2 aspect_ratio) {
...
void main() {
	...
	const vec2 pixelated_uv = pixelate_uv(screenUV, settings.size, aspect_ratio);
	const float dot_matrix = create_dot_matrix(screenUV, settings.size, aspect_ratio);
	const vec4 color = vec4(0, 0, 0, 1);
	fragColor = mix(color, texture(ColorTexture, pixelated_uv), dot_matrix);
}
