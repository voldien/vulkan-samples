...
class ShadowMapWindow : public VKWindow {
  private:
  ...
  public:
	ShadowMapWindow(std::shared_ptr<VulkanCore> &core, std::shared_ptr<VKDevice> &device)
		: VKWindow(core, device, -1, -1, -1, -1) {
		prevTimeCounter = SDL_GetPerformanceCounter();
	}
	~ShadowMapWindow(void) {}
