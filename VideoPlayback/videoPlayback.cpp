...
class AVVideoPlaybackWindow : public VKWindow {
  ...
  public:
	...
	void loadVideo(const char *path) {
		...
		result = avformat_open_input(&this->pformatCtx, path, 0, 0);
		if (result != 0) {
		...
		if ((result = avformat_find_stream_info(this->pformatCtx, NULL)) < 0) {
			char buf[AV_ERROR_MAX_STRING_SIZE];
			av_strerror(result, buf, sizeof(buf));
		...
		if (audio_st) {
			...
			result = avcodec_open2(this->pAudioCtx, audioCodec, NULL);
			if (result < 0) {
				char buf[AV_ERROR_MAX_STRING_SIZE];
				av_strerror(result, buf, sizeof(buf));
				throw std::runtime_error(fmt::format("Failed to retrieve info from stream info : %s", buf));
		...
		if ((result = avcodec_open2(this->pVideoCtx, pVideoCodec, NULL)) != 0) {
			char buf[AV_ERROR_MAX_STRING_SIZE];
			av_strerror(result, buf, sizeof(buf));
			throw std::runtime_error(fmt::format("Failed to retrieve info from stream info : %s", buf));
		}
	...
	virtual void onResize(int width, int height) override {
		...
		for (unsigned int i = 0; i < videoImageViews.size(); i++) {
			...
			VKHelper::createImage(getDevice(), width, height, 1, VK_FORMAT_R8G8B8_UNORM, VK_IMAGE_TILING_LINEAR,
								  VK_IMAGE_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
								  getLogicalDevice()->getPhysicalDevice(0)->getMemoryProperties(), videoStagingFrames[i],
								  videoStagingFrameMemory[i]);
		...
		for (size_t i = 0; i < getCommandBuffers().size(); i++) {
			...
			vkCmdCopyImage(cmd, videoStagingFrames[0], VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, videoFrames[0],
				...
				if (packet.stream_index == this->videoStream) {
					...
					if (result < 0) {
						...
						throw std::runtime_error(
							fmt::format("Failed to send packet for decoding picture frame : %s", buf));
					}
					...
					while (result >= 0) {
						...
						if (this->frame->format != AV_PIX_FMT_BGRA) {
							...
							if (this->frame->format == AV_PIX_FMT_YUV420P) {
								...
								sws_scale(this->sws_ctx, this->frame->data, this->frame->linesize, 0,
										  this->frame->height, this->frameoutput->data, this->frameoutput->linesize);
