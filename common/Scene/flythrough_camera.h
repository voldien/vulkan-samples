#ifndef FLYTHROUGH_CAMERA_H_
#define FLYTHROUGH_CAMERA_H_ 1
...
extern void flythrough_camera_update(float eye[3], float look[3], const float up[3], float view[16],
									 float delta_time_seconds, float eye_speed, float degrees_per_cursor_move,
									 float max_pitch_rotation_degrees, int delta_cursor_x, int delta_cursor_y,
									 int forward_held, int left_held, int backward_held, int right_held, int jump_held,
									 int crouch_held, unsigned int flags);

// Utility for producing a look-to matrix without having to update a camera.
extern void flythrough_camera_look_to(const float eye[3], const float look[3], const float up[3], float view[16],
									  unsigned int flags);
...
void flythrough_camera_update(float eye[3], float look[3], const float up[3], float view[16], float delta_time_seconds,
							  float eye_speed, float degrees_per_cursor_move, float max_pitch_rotation_degrees,
							  int delta_cursor_x, int delta_cursor_y, int forward_held, int left_held,
							  int backward_held, int right_held, int jump_held, int crouch_held, unsigned int flags) {
	...
	float across[3] = {look[1] / look_len * up[2] / up_len - look[2] / look_len * up[1] / up_len,
	...
	float upward[3] = {across[1] * forward[2] - across[2] * forward[1], across[2] * forward[0] - across[0] * forward[2],
	...
	if ((right_held && !left_held) || (!right_held && left_held) || (forward_held && !backward_held) ||
		(!forward_held && backward_held)) {
		...
		float xz_movement[3] = {across[0] * x_multiplier + forward[0] * z_multiplier,
								across[1] * x_multiplier + forward[1] * z_multiplier,
								across[2] * x_multiplier + forward[2] * z_multiplier};

		float xz_movement_len =
			sqrtf(xz_movement[0] * xz_movement[0] + xz_movement[1] * xz_movement[1] + xz_movement[2] * xz_movement[2]);
	...
	if ((jump_held && !crouch_held) || (!jump_held && crouch_held)) {
		...
		float y_movement_len =
			sqrtf(y_movement[0] * y_movement[0] + y_movement[1] * y_movement[1] + y_movement[2] * y_movement[2]);
	...
	if (delta_cursor_x != 0) {
		...
		float yaw_rotation[9] = {yaw_cos + (1.0f - yaw_cos) * up_norm[0] * up_norm[0],
		...
		float newlook[3] = {yaw_rotation[0] * look[0] + yaw_rotation[3] * look[1] + yaw_rotation[6] * look[2],
	...
	if (delta_cursor_y != 0) {
		...
		float pitch_rotation[9] = {pitch_cos + (1.0f - pitch_cos) * across[0] * across[0],
		...
		float newlook[3] = {pitch_rotation[0] * look[0] + pitch_rotation[3] * look[1] + pitch_rotation[6] * look[2],
...
void flythrough_camera_look_to(const float eye[3], const float look[3], const float up[3], float view[16],
	...
	float s[3] = {f[1] * up_norm[2] - f[2] * up_norm[1], f[2] * up_norm[0] - f[0] * up_norm[2],
	...
	if (!(flags & FLYTHROUGH_CAMERA_LEFT_HANDED_BIT)) {
		...
		// (notice the last row of gluPerspective, which makes it back into a left-handed system after perspective
		// division)
		f[0] = -f[0];
		f[1] = -f[1];
		f[2] = -f[2];
	...
	// t = [s;u;f] * -eye
	float t[3] = {s[0] * -eye[0] + s[1] * -eye[1] + s[2] * -eye[2], u[0] * -eye[0] + u[1] * -eye[1] + u[2] * -eye[2],
